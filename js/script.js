...
for (var i = 0; i < text.question.length; i++) {
  ...
  var question = document.createElement ('h4');
      question.innerHTML = text.question [i];

  listItem.appendChild (question);
  for (var j = 0; j < 3; j++) {
    var checkbox = document.createElement ('input');
      checkbox.type = 'checkbox';
      checkbox.name = text.checkboxName [k];
      checkbox.id = text.id [k];

    var answer = document.createElement ('p');
        answer.style.marginBottom = '0';

    var label = document.createElement ('label');
      label.htmlFor = text.id [k];
      label.style.marginLeft = '10px';
      label.style.fontSize = '18px';
      label.style.fontWeight = 'normal';

    label.appendChild (document.createTextNode(text.answer [k]));
